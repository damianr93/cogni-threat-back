...
export class ActorsService {
    ...
    async update(id: string, data: UpdateActorDto) {
        try {
            ...
            if (data.hitosDatas) {
                const incomingIds = data.hitosDatas.filter(h => h.id).map(h => h.id);
            ...
            const actor = await this.prisma.actorsData.update({
                ...
                data: {
                    ...
                    ...(data.identificatedDate !== undefined && { identificatedDate: data.identificatedDate }),
                    ...(data.subGroup !== undefined && { subGroup: data.subGroup }),
                    ...(data.description !== undefined && { description: data.description }),
                    ...(data.country !== undefined && { country: data.country }),
                    ...(data.descriptionMethods !== undefined && { descriptionMethods: data.descriptionMethods }),
            ...
            const where = q
                ? {
                    OR: [
                        ...
                        { carriedOutBy: { name: { contains: q, mode: 'insensitive' as const } } },
                    ],
                }
                : {};
    ...
    actor.hitosDatas.forEach((hito) => {
        ...
        doc.fontSize(12).text(
            `${hito.date?.toISOString().split('T')[0] ?? ''}`
        );
        ...
        sections: [
            {
                children: [
                    ...
                    new Paragraph(
                        `Métodos: ${actor.methods.join(', ') || '-'}`
                    ),
                    new Paragraph(
                        `Descripción métodos: ${
                            actor.descriptionMethods ?? '-'
                        }`
                    ),
