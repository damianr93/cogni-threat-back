...
export class RansomwareDataProcessorService {
  processVictimData(rawData: any) {
    return {
      discovered: rawData.discovered
        ? new Date(rawData.discovered)
        : new Date(),
  ...
  processGroupData(rawData: any) {
    ...
    return {
      ...
      negotiation_count:
        typeof rawData.negotiation_count === 'number'
          ? rawData.negotiation_count
          : 0,
      has_ransomnote: rawData.has_ransomnote ?? false,
      ransomnotes_count:
        typeof rawData.ransomnotes_count === 'number'
          ? rawData.ransomnotes_count
          : 0,
      url: typeof rawData.url === 'string' ? rawData.url.trim() : null,
      ttps: Array.isArray(rawData.ttps)
        ? rawData.ttps.map((ttp: any) =>
            typeof ttp === 'string' ? ttp : JSON.stringify(ttp),
          )
        : [],
      vulnerabilities: Array.isArray(rawData.vulnerabilities)
        ? rawData.vulnerabilities.map((vuln: any) =>
  ...
  private resolveAltName(data: any): string {
    if (typeof data?.altname === 'string' && data.altname.trim().length > 0)
      return data.altname.trim();
    if (Array.isArray(data?.aliases) && data.aliases.length > 0)
      return String(data.aliases[0]);
    return 'N/A';
  }

  private resolveVictims(data: any): number {
    if (typeof data?.victims === 'number' && Number.isFinite(data.victims))
      return data.victims;
    if (
      typeof data?.victims_count === 'number' &&
      Number.isFinite(data.victims_count)
    )
      return data.victims_count;
    if (
      typeof data?.total_victims === 'number' &&
      Number.isFinite(data.total_victims)
    )
      return data.total_victims;
    return 0;
  }
}
