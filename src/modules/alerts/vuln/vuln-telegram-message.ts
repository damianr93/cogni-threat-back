...
export function buildVulnTelegramMessage(params: {
  ...
  if (params.title)
    lines.push(`<b>Título:</b> ${params.escapeHtml(params.title)}`);
  ...
  lines.push(
    `<b>Severidad:</b> ${params.escapeHtml(params.severity ?? 'UNKNOWN')} (CVSS ${params.cvssScore})`,
  );
  if (params.epssScore) {
    lines.push(
      `<b>EPSS:</b> ${params.epssScore} (${params.epssPercentile}% percentil)`,
    );
  }
  ...
  if (params.description) {
    lines.push(
      `<b>Descripción:</b> ${params.escapeHtml(params.truncate(params.description, 200))}...`,
    );
  }

  const links: string[] = [];
  if (params.nvdLink) links.push(`<a href="${params.nvdLink}">NVD</a>`);
  if (params.dashboardLink)
    links.push(`<a href="${params.dashboardLink}">Dashboard</a>`);
...
function groupHitsByProfile(hits: ProfileMatchHit[]): string[] {
  ...
  return Array.from(map.entries()).map(
    ([profile, items]) => `${profile}: ${items.join(', ')}`,
  );
}

function summarizePackages(affectedPackages: unknown): string | null {
  if (!Array.isArray(affectedPackages) || affectedPackages.length === 0)
    return null;
  ...
  const suffix =
    affectedPackages.length > 3 ? ` (+${affectedPackages.length - 3})` : '';
